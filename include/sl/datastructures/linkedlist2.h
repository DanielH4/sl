#ifndef _SL_DATASTRUCT_LINKEDLIST_
#define _SL_DATASTRUCT_LINKEDLIST_
...
void sl_linkedlist_add(sl_linkedlist_t* list, void* data, size_t nbytes) {
	//sl_linkedlist_insert(list, data, type_name, nbytes, 0);
...
void sl_linkedlist_print(sl_linkedlist_t* list, void (*print_func)(const sl_node_t* node)) {
	sl_node_t *pnode = list->head;
	while(pnode) {
		print_func(pnode);
		pnode = pnode->next;	
...
#endif //_SL_DATASTRUCT_LINKEDLIST_
